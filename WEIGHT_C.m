function [weight]=WEIGHT_C(NUM_DMs,DD)
I=NUM_DMs;
...
omega=zeros(NUM_DMs,1); %weight is the weight vector of DMs
for iii=1:NUM_DMs
    omega(iii,1)=(1-OC(iii)/sum(OC)-BC(iii)/sum(BC))/(NUM_DMs-2); % compute every weight for each DM  
end
weight=omega';
end